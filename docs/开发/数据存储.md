...
`backend.core.storage` 模块提供本地数据持久化功能，支持：
- 成绩数据 CSV 存储
- 登录配置 JSON 存储
- Session 持久化
- 自动目录管理
...
默认存储目录：`./data/` (项目根目录，空则自动使用当前目录下的 `data/`)
...
├── session.json                # CAS/WebVPN Cookie 会话（敏感）
...
```python
from backend.core.storage import AcademicStorage
...
```python
from backend.core.storage import AcademicReportStorage
...
```python
from backend.core.storage import AutoLoginManager
...
```python
from backend.core.storage import quick_save
...
```python
# backend/core/auth/client.py 中添加
...
2. **Session 文件**：`session.json` 包含敏感 Cookie，不要上传到版本控制或分享
3. **数据目录**：默认存储在项目根目录的 `data/`，多用户系统注意目录权限

**建议：**
- 生产环境使用环境变量或密钥管理服务
- 将 `./data/` 添加到 `.gitignore`
- 定期清理旧的 session 文件
