...
前端资源已在发行时生成 gzip/Brotli 版本，内置 FastAPI 会按浏览器能力返回压缩内容，并让
带版本哈希的 JS/CSS 长期缓存、HTML 每次重新验证。用户无需安装或配置 Nginx/Caddy，也无需
清理浏览器缓存来完成正常升级；新 HTML 会引用新哈希资源。运行载荷不包含 source map。
