...
1. 从 GitHub Releases 下载 `NEU-JWXT-Toolkit-1.3.0-windows-x64-setup.exe`。
...
下载 `NEU-JWXT-Toolkit-1.3.0-windows-x64-portable.zip`，完整解压后运行 `NEU-JWXT-Toolkit.exe`。不要只从压缩包内直接运行单个 EXE；同目录的 `_internal` 文件也是运行所必需的。
