...
这两页是两套不同凭据。程序已使用不同的表单名、字段名和浏览器自动填充分区，防止移动浏览器
或密码管理器把网站访问密码误填为 NEU 账号密码。若浏览器在升级前已经保存了错误的站点填充
记录，应在该域名的密码管理器中删除旧记录，再分别保存网站访问密码和 NEU 账号密码。

退出时前端会立即回到登录页，后端清理可能仍需等待一条已经开始的学校请求结束；等待期间新的
登录会排在清理之后，不会被旧登出反向清除。若服务器日志中 `/api/status` 或 `/api/logout`
持续耗时数十秒，应同时检查服务器到学校系统/WebVPN 的网络质量，但不应通过增加后端进程绕过：
当前认证 Flow 和共享 Session 仍按单进程设计。
