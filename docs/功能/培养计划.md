...
要求 7 学分，而科学素养声明要求仅为 4 学分。若直接按子组声明值
封顶，父组最多只能计入 11 学分。

当一个选修父组存在未分配要求学分，并且只有一个“包含内部子组且
自身有学分要求”的弹性子组时，使用以下有效上限：

```text
弹性子组有效要求 = 父组要求 - 其他直接子组声明要求之和
```

该样本中科学素养的有效要求为 `13 - 7 = 6`。接口原始值仍保存在
`declared_required_credits`，派生增加值保存在 `requirement_adjustment`，
页面和父级汇总使用 `required_credits` 的有效值。此规则依据树和规则
字段触发，不依赖“科学素养类”这一固定名称。
