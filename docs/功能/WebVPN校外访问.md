...
| 模式 | 适用网络 | 认证方式 | 说明 |
| --- | --- | --- | --- |
| `direct` | 校园网 | 学号和密码 | 直接访问教务系统。主动登录失败时给出线路提示；静默恢复确认直连不可达后自动尝试 WebVPN。 |
| `webvpn` | 校外网络 | 微信扫码（推荐）或账号密码 | 所有目标为 `*.neu.edu.cn` 的业务请求会自动转为 WebVPN 地址。 |

登录页仍由用户明确选择校内直连或 WebVPN，不根据模糊网络特征猜测环境。进程内静默恢复只对可验证的失败自动切线：直连连接失败、超时或被导向 WebVPN 时尝试 WebVPN；WebVPN 明确返回校园网拒绝时尝试直连。密码错误不会换线重复提交，普通业务错误也不会触发切线。
...
| `backend/core/auth/keepalive.py` | 低频会话核验、恢复唤醒和进程生命周期内的非阻塞保活。 |
...
| 方法和路径 | 请求字段 | 关键响应字段 |
| --- | --- | --- |
| `GET /api/status` | 无 | `is_logged_in`、`has_credentials`、`has_local_data`、`current_user`、`network_mode`、`storage`、`recovery`、`pending_auth` |
...
Cookie 和已保存账密的自动恢复由进程级认证管理器统一单航班和退避。一次完整恢复链路失败后，
所有页面、缓存刷新和后台任务共享同一个冷却状态，首次等待 5 秒，连续失败逐步延长，最长 2 分钟；
状态读取只返回本地恢复快照，不在浏览器请求内排队执行 CAS 网络访问。成功登录、显式退出或切换账号
会清除该状态。若恢复进入图形验证码/短信阶段，则保留产生挑战的原 Session 等待用户处理，不再按
定时轮询重新提交账密，也不会自动发送短信。

`recovery.status` 包含 `idle`、`recovering`、`authenticated`、`retry_wait`、
`interaction_required`、`credentials_invalid` 和 `manual_required`。只有保存密码已被明确拒绝、没有
可用凭据，或学校要求验证码/短信等用户操作时才进入人工路径；网络超时和临时上游失败保持自动恢复
与退避，不立即弹出重新登录对话框。
...
静默恢复失败后会保留当前页面上下文并弹出重新登录入口；登录成功后返回原页面。可用缓存时仍可选择只读离线模式，手动退出登录不会触发自动恢复。
...
“校内直连”，不会替用户重放正在进行的交互式登录。已有 WebVPN 会话在静默恢复中遇到该错误时，
保留前台页面和已有缓存，并在存在保存凭据时自动尝试校内直连；验证码、短信和远端写操作不会
自动重放。选课页仍提供“直连”和“跟随教务”服务级线路切换。
...
应用启动后由保活服务立即唤醒一次恢复，运行中约每 90 秒低频核验活动会话。恢复优先检查内存会话，再尝试 `session.json` 中的 Cookie 和同账号保存凭据；明确线路故障时自动切换直连/WebVPN。WebVPN 恢复若进入短信验证，会把产生挑战的同一个客户端保存为“待认证候选 Session”并交给前台弹窗；后续页面读取和后台任务等待该 Flow 完成、取消或过期，不得再次提交密码表单或替换验证码。系统不会自动发送短信。Flow 过期或用户取消后，才允许下一次正常恢复。

可见只读请求发现重定向或 HTTP 200 登录表单时立即返回统一认证状态，不在该请求中串行等待完整登录链；前端观察本地恢复状态，成功后只重试原只读请求一次。写操作和子会话建立仍在后端独占认证边界内完成，写请求不会被前端自动重放。只有恢复状态确认必须人工处理时，前端才提供重新登录或只读离线模式；该降级不会调用清理数据的登出接口，也不会删除凭据、缓存或用户文档。
...
手动密码、扫码或短信登录成功会推进前端认证代际并销毁旧的失效提示。此前尚未完成的 `401`
恢复结果不得再弹出“教务会话已失效”；它会按新会话重试原只读请求一次。登录页状态探测与用户
提交恰好同时成功时，成功状态和提示按同一键幂等提交，避免重复 Toast 或成功后仍覆盖出失败弹窗。
