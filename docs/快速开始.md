...
│
├── docs/                        # 开发文档
├── data/                        # 本地数据、会话和运行日志（运行后生成）
└── requirements.txt             # Python 依赖
```

## 2. 发行版安装（普通用户推荐）

- Windows 用户请使用 [Windows 本地安装](部署/Windows本地安装.md)，无需预装 Python 或 Node.js。
- 将 Linux 主机作为服务端时，请使用 [Linux 轻量服务](部署/Linux轻量服务.md)，并自行配置 HTTPS 反向代理。
...
首次运行时脚本会自动创建 `.venv`、安装 `requirements.txt` 中的 Python 依赖，并按需要构建前端。`requirements.txt` 使用 ASCII 注释，可避免中文 Windows 中 pip 按 GBK 读取依赖清单时报解码错误。
...
```bash
# 首次会自动创建 Python 环境、安装依赖并构建前端（如需强制重新构建，加 --build）
python start_all.py
...
完整的校外访问、二维码/短信认证、会话恢复和认证接口说明见 [WebVPN 校外访问](功能/WebVPN校外访问.md)。
成绩变化定时检查与邮件通知配置见 [成绩追踪](功能/成绩追踪.md)。
...
- **校内直连**：在校园网络中输入学号和密码。
- **WebVPN**：校外网络请选择 WebVPN。优先使用“微信扫码快速登录”；账号密码登录可能进入短信二次认证，输入验证码后才会完成登录。
- 直连请求缓慢、超时或被重定向到认证页时，页面会提示检查校园网络或切换 WebVPN。
...
### 数据存储位置
- 源码开发模式存储在项目根目录的 `data/` 文件夹中
- Windows 发行版存储在 `%LOCALAPPDATA%\NEU-JWXT-Toolkit\data`
- Linux 服务版存储在 `/var/lib/neu-jwxt-toolkit`
- 包含：成绩 CSV、追踪快照和配置 JSON、登录凭证、日志
...
│  Cookie 持久化机制                                            │
│  ────────────────                                             │
│  1. 保存 CAS/WebVPN Cookie 到本地文件 (data/session.json)     │
...
client = NEUAuthClient(
    username="学号",
    password="密码",
    cookie_file="data/session.json"  # 启用 Cookie 持久化
)
...
4. **文件权限** - `session.json` 包含敏感信息，注意文件权限设置
