...
| 查询课表 | 我的、班级、教师和教室课表，周/学期视图及统一冲突检测；四类课表共享手机缩略网格，个人课表支持快照首屏、当日日程与调休复制，教室支持增量空闲筛选 |
...
- 选课、退课、投权、报名、取消报名、评教和证明生成都属于远端写操作。界面会在提交前
  保留确认步骤；响应不明确时不会自动重放。
- 四节活动的“创院系统线路”支持跟随教务、直连和 WebVPN，默认跟随教务。活动列表、
  详情和证书下载共用线路；创院认证可在当前页面通过账号密码、扫码或短信恢复，
  不改变教务主线路，也不清除已保存活动。
